			...
			<table>
				...
				<?php
					try
					{
						...
						$stmt=$connect->prepare("SELECT House.id, House.name, House.price, House.location, House.time, Account.name AS user FROM Favorite, House, Account WHERE Favorite.user_id = :owner_id AND Favorite.favorite_id = House.id AND House.owner_id = Account.id");
